...
void Presenter_StopWatch_LCD(stopWatch_t stopWatchData)
{
	...
	if (state == S_STOPWATCH_CLEAR && clearDispFlag == 0) {
		clearDispTime = HAL_GetTick();
		clearDispFlag = 1;
		sprintf(str2, "CLEAR!!");
	}
	else if (clearDispFlag) {
		if (HAL_GetTick() - clearDispTime >= 2000) {
			clearDispFlag = 0;
		}
	}
	else {
		if (state == S_STOPWATCH_RUN) {
			sprintf(str2, "RUNNING");
		}
		else if (state == S_STOPWATCH_STOP) {
			sprintf(str2, "STOPPED");
		}
	}
