...
void Button_Init(Button_TypeDef *hbtn, GPIO_TypeDef *GPIOx, uint32_t pinNum)
{
	hbtn->GPIOx = GPIOx;
	hbtn->pinNum = pinNum;
...
{
    uint32_t curState = GPIO_ReadPin(hbtn->GPIOx, hbtn->pinNum);

    button_state_t result = NO_ACT;

    if (hbtn->prevState == BUTTON_RELEASED && curState == BUTTON_PRESSED) {
		delay(1); // debounce
		if (GPIO_ReadPin(hbtn->GPIOx, hbtn->pinNum) == BUTTON_PRESSED) {
			result = ACT_PUSHED;
    	}
    }
    else if (hbtn->prevState == BUTTON_PRESSED && curState == BUTTON_RELEASED) {
		delay(1); // debounce
		if (GPIO_ReadPin(hbtn->GPIOx, hbtn->pinNum) == BUTTON_RELEASED) {
			result = ACT_RELEASED;
    	}
    }
